  ...
  def show
    ...
    if @campaign.is_seo?
      ...
      @daily_keyword_ranking_graph = HighChart.new('graph') do |f|
        ...
        f.series(:name=> 'Google Ranking', :marker => {:enabled => false, :states => {:hover => {:enabled => true, :symbol => "circle", :radius => "5", :lineWidth => "1"}}}, :fillOpacity => '.3', :pointInterval => 24 * 3600 * 1000, :pointStart => @start_date.to_time_in_current_zone.at_beginning_of_day.utc.to_i * 1000, :data => @keyword.daily_most_recent_google_ranking_between(@start_date, @end_date))
        f.series(:name=> 'Yahoo Ranking', :marker => {:enabled => false, :states => {:hover => {:enabled => true, :symbol => "circle", :radius => "5", :lineWidth => "1"}}}, :fillOpacity => '.3', :pointInterval => 24 * 3600 * 1000, :pointStart => @start_date.to_time_in_current_zone.at_beginning_of_day.utc.to_i * 1000, :data => @keyword.daily_most_recent_yahoo_ranking_between(@start_date, @end_date))
        #f.series(:name=> 'Bing Ranking', :marker => {:enabled => false, :states => {:hover => {:enabled => true, :symbol => "circle", :radius => "5", :lineWidth => "1"}}}, :fillOpacity => '.3', :pointInterval => 24 * 3600 * 1000, :pointStart => @start_date.to_time_in_current_zone.at_beginning_of_day.utc.to_i * 1000, :data => @keyword.daily_most_recent_bing_ranking_between(@start_date, @end_date))
      end
    end
